		...
		<div id="list-society" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="code" title="${message(code: 'society.code.label', default: 'Code')}" />
					
						<g:sortableColumn property="nome" title="${message(code: 'society.nome.label', default: 'Nome')}" />
				...
				<g:each in="${societyInstanceList}" status="i" var="societyInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${societyInstance.id}">${fieldValue(bean: societyInstance, field: "code")}</g:link></td>
					
						<td>${fieldValue(bean: societyInstance, field: "nome")}</td>
