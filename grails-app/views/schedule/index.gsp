		...
		<div id="list-schedule" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="isAvaliable" title="${message(code: 'schedule.isAvaliable.label', default: 'Is Avaliable')}" />
					
						<g:sortableColumn property="time" title="${message(code: 'schedule.time.label', default: 'Time')}" />
				...
				<g:each in="${scheduleInstanceList}" status="i" var="scheduleInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${scheduleInstance.id}">${fieldValue(bean: scheduleInstance, field: "isAvaliable")}</g:link></td>
					
						<td>${fieldValue(bean: scheduleInstance, field: "time")}</td>
